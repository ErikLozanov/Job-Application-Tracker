...
const StatusBadge = ({ status }: { status: string }) => {
    // Preserving YOUR exact original color mapping
...
const JobDetails = () => {
    ...
    const handleGenerateAI = () => {
        generateAI(id!, { onSuccess: (data) => setGeneratedLetter(data.coverLetter) });
    };

    const handleGeneratePrep = () => {
        generateInterview(id!, { onSuccess: (data) => setInterviewPrep(data.interviewPrep) });
    };
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-8 pb-20">
            ...
            <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-lg border border-white/50 p-6 sm:p-8 relative overflow-hidden">
                {/* Keeping your exact original gradient overlay */}
                                ...
                                {isEditing ? (
                                    <div className="space-y-2 max-w-md">
                                        <input
                                            ...
                                            className="block w-full text-2xl font-bold text-gray-900 bg-transparent border-b border-gray-300 focus:border-blue-500 focus:ring-0 px-0 placeholder-gray-400 transition-all"
                                            placeholder="Company"
                                        />
                                        <input
                                            type="text" name="jobTitle" value={formData.jobTitle} onChange={handleChange}
                                            className="block w-full text-lg text-gray-600 bg-transparent border-b border-gray-300 focus:border-blue-500 focus:ring-0 px-0 placeholder-gray-400 transition-all"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    {isEditing && (
                        <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-sm border border-white/50 p-6 space-y-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                ...
                                {formData.status === 'INTERVIEW' && (
                                    <div className="animate-fade-in">
                                        <label className="block text-sm font-bold text-amber-600 mb-1">Interview Date</label>
                                        <input type="date" name="interviewDate" value={formData.interviewDate} onChange={handleChange} className="block w-full rounded-lg border-amber-200 bg-amber-50 p-2.5 text-sm focus:ring-amber-500 focus:border-amber-500" />
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="pt-4 border-t border-gray-100">
                                ...
                                <label className="cursor-pointer flex items-center justify-center w-full p-4 border-2 border-dashed border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                    ...
                    {/* Notes Section */}
                    <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-sm border border-white/50 p-6 h-full flex flex-col">
                        <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-4 flex items-center gap-2">
                            <span className="text-lg">📝</span> Notes & Strategy
                        </h3>
                        {isEditing ? (
                            <textarea
                                name="notes" rows={12} value={formData.notes} onChange={handleChange}
                            ...
                            <div className="prose prose-sm max-w-none text-gray-600 whitespace-pre-wrap leading-relaxed bg-gray-50/50 p-4 rounded-xl border border-gray-100 min-h-[12rem]">
                ...
                {/* RIGHT COLUMN: Tools (Sticky for better UX) */}
                <div className="space-y-6 lg:sticky lg:top-24 h-fit">
                    
                    {/* External Link */}
                    {!isEditing && job.jobUrl && (
                        <a href={job.jobUrl} target="_blank" rel="noopener noreferrer" className="block w-full text-center py-3 px-4 rounded-xl bg-white border border-gray-200 text-blue-600 font-bold hover:bg-blue-50 hover:border-blue-200 transition-all shadow-sm">
                            Open Job Posting ↗
                        </a>
                    )}
                    ...
                    {!isEditing && (
                        job.resumeUrl ? (
                            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 relative overflow-hidden group hover:border-blue-200 transition-colors">
                                <div className="flex items-center gap-4">
                                    <div className="p-3 bg-red-50 rounded-lg group-hover:scale-110 transition-transform duration-300">
                                ...
                                <a href={job.resumeUrl} target="_blank" rel="noopener noreferrer" className="mt-4 flex items-center justify-center w-full py-2.5 text-sm font-bold text-gray-700 bg-gray-50 hover:bg-gray-100 border border-gray-200 rounded-xl transition-colors">
                                    View Document
                                </a>
                            </div>
                        ) : (
                            <div className="bg-amber-50 rounded-2xl shadow-sm border border-amber-200 p-6">
                                <div className="flex items-center gap-3 mb-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-amber-700 mt-1">Attach resume for better AI results.</p>
                                    </div>
                                </div>
                                <button onClick={() => setIsEditing(true)} className="w-full py-2 text-xs font-bold text-amber-700 bg-white border border-amber-200 rounded-lg hover:bg-amber-50 transition-colors">Upload Resume</button>
                            </div>
                        )
                    )}

                    {/* Google Calendar (Only if Interview) */}
                    ...
                    {/* --- AI: Cover Letter --- */}
                    <div className="bg-gradient-to-b from-indigo-50 to-white rounded-2xl shadow-sm border border-indigo-100 p-6 relative overflow-hidden">
                    ...
                    {/* --- AI: Interview Coach --- */}
                    <div className="bg-gradient-to-b from-emerald-50 to-white rounded-2xl shadow-sm border border-emerald-100 p-6 relative overflow-hidden">
                        <div className="absolute top-0 right-0 w-20 h-20 bg-emerald-200 rounded-full blur-2xl opacity-50 -z-10" />
                        <div className="flex items-center gap-2 mb-4">
                            <span className="text-2xl">🎤</span>
                        ...
                        {!interviewPrep ? (
                            <>
                                <p className="text-sm text-emerald-700/80 mb-6 leading-relaxed">
                                    Get customized interview questions and tips based on this specific role and your resume.
                                </p>
                                <button onClick={handleGeneratePrep} disabled={isPrepLoading} className="w-full py-3 rounded-xl bg-emerald-600 text-white font-bold shadow-lg shadow-emerald-500/30 hover:bg-emerald-700 hover:-translate-y-0.5 transition-all disabled:opacity-50">
                                    {isPrepLoading ? <span className="animate-pulse">Preparing...</span> : 'Start Coaching'}
                                </button>
                            </>
                        ) : (
                            <div className="space-y-3 animate-fade-in">
                                <div className="w-full h-80 overflow-y-auto text-xs text-gray-700 bg-white border border-emerald-100 rounded-lg p-4 leading-relaxed whitespace-pre-wrap">
                                    {interviewPrep}
                                </div>
                                <div className="flex gap-2">
                                    <button onClick={() => setInterviewPrep('')} className="flex-1 py-2 text-xs font-bold text-gray-500 hover:text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200">Close</button>
                                    <button onClick={() => { navigator.clipboard.writeText(interviewPrep); alert('Copied!'); }} className="flex-1 py-2 text-xs font-bold text-emerald-700 bg-emerald-100 rounded-lg hover:bg-emerald-200 border border-emerald-200">Copy</button>
                                </div>
                            </div>
                        )}
                    </div>
